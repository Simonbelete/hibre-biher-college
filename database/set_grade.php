        ...
        if(isset($_POST['Submit']))
        {
            ...
            if($action=='Register'){
                if(mysqli_query($link, $sql)){
                  header("Location:../General_set_grade.php?grade_err=insert");  //student id is already present
                }
                else{
                    header("Location:../General_set_grade.php?grade_err=notinsert");
                }
            }
            if($action=='Update'){
                            if(mysqli_query($link, $sql2)){
                                mysqli_query($link, $sql3);
                                header("Location:../General_set_grade.php?grade_err=update");  //student id is already present
                            }
                            else{
                                    header("Location:../General_set_grade.php?grade_err=".mysqli_error($link));
